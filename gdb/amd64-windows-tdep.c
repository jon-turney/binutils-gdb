...
struct amd64_cygwin_sigwrapper_frame_cache
{
  CORE_ADDR sp;
  CORE_ADDR pc;
  CORE_ADDR prev_pc;
...
/* Fill THIS_CACHE using the amd64-cygwin sigwrapper unwinding data
   for THIS_FRAME.  */

static struct amd64_cygwin_sigwrapper_frame_cache *
amd64_cygwin_sigwrapper_frame_cache (struct frame_info *this_frame, void **this_cache)
{
  struct gdbarch *gdbarch = get_frame_arch (this_frame);
  enum bfd_endian byte_order = gdbarch_byte_order (gdbarch);
  struct amd64_cygwin_sigwrapper_frame_cache *cache;
  ptid_t ptid;
  CORE_ADDR pc;
  ...
  cache = FRAME_OBSTACK_ZALLOC (struct amd64_cygwin_sigwrapper_frame_cache);
  *this_cache = cache;
  ...
  */
  ...
  read_memory(thread_local_base + 8, buf, 8);
  stacktop = extract_unsigned_integer(buf, 8, byte_order);

  if (frame_debug)
    fprintf_unfiltered (gdb_stdlog,
			"amd64_cygwin_sigwrapper_frame_prev_register TEB.stacktop=%s\n",
			paddress (gdbarch, stacktop ));

  /* Find cygtls, relative to stacktop, read signalstackptr from cygtls */
  read_memory(stacktop - 0x1da0, buf, 8);
  signalstackptr = extract_unsigned_integer(buf, 8, byte_order);

  if (frame_debug)
    fprintf_unfiltered (gdb_stdlog,
			"amd64_cygwin_sigwrapper_frame_prev_register sigsp=%s\n",
			paddress (gdbarch, signalstackptr));

  /* Read return address from signal stack */
  read_memory(signalstackptr - 8, buf, 8);
  cache->prev_pc = extract_unsigned_integer(buf, 8, byte_order);

  if (frame_debug)
    fprintf_unfiltered (gdb_stdlog,
			"amd64_cygwin_sigwrapper_frame_prev_register ra=%s\n",
...
static struct value *
amd64_cygwin_sigwrapper_frame_prev_register (struct frame_info *this_frame, void **this_cache,
					     int regnum)
{
  struct gdbarch *gdbarch = get_frame_arch (this_frame);
  struct amd64_cygwin_sigwrapper_frame_cache *cache =
    amd64_cygwin_sigwrapper_frame_cache (this_frame, this_cache);

  if (frame_debug)
    fprintf_unfiltered (gdb_stdlog,
			"amd64_cygwin_sigwrapper_frame_prev_register %s for pc=%s\n",
			gdbarch_register_name (gdbarch, regnum),
			paddress (gdbarch, cache->prev_pc));
...
static void
amd64_cygwin_sigwrapper_frame_this_id (struct frame_info *this_frame, void **this_cache,
				      struct frame_id *this_id)
{
  struct amd64_cygwin_sigwrapper_frame_cache *cache =
    amd64_cygwin_sigwrapper_frame_cache (this_frame, this_cache);
...
static int
amd64_in_sigwrapper_p (CORE_ADDR pc)
{
  const char *name;

  find_pc_partial_function (pc, &name, NULL, NULL);

  return name && (strcmp(name, "_sigbe") == 0);
}

static int
amd64_cygwin_sigwrapper_frame_sniffer (const struct frame_unwind *self,
				       struct frame_info *this_frame,
				       void **this_cache)
{
  return amd64_in_sigwrapper_p (get_frame_pc (this_frame));
}

/* Cygwin sigwapper unwinder.  */

static const struct frame_unwind amd64_cygwin_sigwrapper_frame_unwind =
{
  NORMAL_FRAME,
  default_frame_unwind_stop_reason,
  &amd64_cygwin_sigwrapper_frame_this_id,
  &amd64_cygwin_sigwrapper_frame_prev_register,
  NULL,
  &amd64_cygwin_sigwrapper_frame_sniffer
};
